...
Changes to the list made when a drop is performed can be listened to using
the `update:data` event. The newly-sorted array will be passed as a parameter.
...
export const Template = (args, { argTypes }) => ({
  ...
  template: `
    <div style="border: 1px dashed #7B61FF; border-radius: 5px; width: 300px;">
      <PbDragAndDrop :data.sync="actions" :show-index="showIndex">
        <template #item="{ item }">
